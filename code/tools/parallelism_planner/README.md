...
python -m tools.parallelism_planner recommend llama-3.1-70b

# Training configuration
python -m tools.parallelism_planner recommend llama-3.1-405b --training --batch-size 32

# Get sharding recommendations (ZeRO/FSDP)
python -m tools.parallelism_planner sharding llama-3.1-70b --dp 8 --memory 80

# Generate launch commands
python -m tools.parallelism_planner launch --nodes 2 --gpus 8 --tp 4 --pp 2 --dp 2

# Validate configuration before running
python -m tools.parallelism_planner validate llama-3.1-70b --tp 8 --dp 1

# Get advanced optimizations
python -m tools.parallelism_planner optimize llama-3.1-70b --goal throughput

# Get workload-specific profile
python -m tools.parallelism_planner profile llama-3.1-70b --workload pretraining

# Get profile with LoRA for fine-tuning
python -m tools.parallelism_planner profile llama-3.1-8b --workload finetuning --lora

# List available profiles
python -m tools.parallelism_planner profile --list

# Cost/throughput Pareto analysis
python -m tools.parallelism_planner pareto llama-3.1-70b --gpu-cost 4.0

# Generate SLURM job script
python -m tools.parallelism_planner slurm --job-name llama-train --nodes 4 --gpus 8

# Estimate training time/cost
python -m tools.parallelism_planner estimate llama-3.1-70b --tokens 1e12 --gpus 64

# Compare multiple models
python -m tools.parallelism_planner compare llama-3.1-8b llama-3.1-70b mixtral-8x7b

# Use mock topology (no GPU required)
python -m tools.parallelism_planner recommend llama-3.1-70b --mock-topology 8xB200

# Output as JSON
python -m tools.parallelism_planner recommend mixtral-8x7b --json
```
...
```python
from tools.parallelism_planner import validate_full_configuration
...
```python
from tools.parallelism_planner import get_advanced_optimization_report
...
```python
from tools.parallelism_planner import get_performance_profile, list_available_profiles
...
- `common/python/hardware_capabilities.py` - Hardware detection
- `tools/dashboard/server.py` - Web UI and API
