...
- **FlexAttention vmap fix**: The causal mask now relies on tensor boolean ops
  instead of Python `if` statements, resolving the `"data-dependent control flow
  not supported"` error surfaced by `torch.vmap`. When authoring custom block
  masks, keep the comparison in tensor space and pass the `device` argument
  through to `create_block_mask(..., device=device)` so multi-GPU ranks build
  their mask on the correct accelerator. Validate changes with
  `python ch16/test_gpt_large_optimized.py --tensor-parallel-gpus 8 --attention-backend flex`
  and inspect the emitted JSON for `max_abs_diff: 0.0`.
