// Baseline double buffering binary.
...
int main() {
    ...
    for (int i = 0; i < 5; ++i) {
        double_buffer_baseline_kernel<<<double_buffer_grid(elements), kDoubleBufferBlock, shared_bytes>>>(d_input, d_output, elements);
    }
    ...
    std::cout << "Baseline double buffering: " << (total_ms / iterations) << " ms\n";
