...
**Targets:** NVIDIA B200/B300 (SM 10.0), CUDA 13.0, PyTorch 2.9, Triton 3.5, CUTLASS 4.2.1
...
- PyTorch 2.9/Triton 3.5 optimizations remain enabled by default for Blackwell deployments.
- CUTLASS 4.2.1 backend configured via `nvidia-cutlass-dsl` (no local clone required).
...
- **Automation:** CUTLASS 4.2.1 is provided through the `nvidia-cutlass-dsl` wheel (`setup.sh:889`), and future feature tracking stays current in `docs/future_optimizations.md:1`.
