...
const MAX_DISPLAY_SPEEDUP = 1000;
const MIN_DISPLAY_SPEEDUP_CAP = 10;
const DEFAULT_SPEEDUP_HEADROOM = 50;

function computeSpeedupCap(benchmarks: Benchmark[]): number {
  const speedups = benchmarks
    .map((b) => b.speedup)
    .filter((s): s is number => typeof s === 'number' && isFinite(s) && s > 0)
    .sort((a, b) => a - b);

  if (speedups.length === 0) return MAX_DISPLAY_SPEEDUP;

  const p95Index = Math.min(speedups.length - 1, Math.floor(speedups.length * 0.95));
  const p95 = speedups[p95Index];
  const median = speedups[Math.floor(speedups.length / 2)];
  const headroom = Math.max(p95 * 1.25, median * 6, DEFAULT_SPEEDUP_HEADROOM);

  return Math.min(Math.max(headroom, MIN_DISPLAY_SPEEDUP_CAP), MAX_DISPLAY_SPEEDUP);
}

function normalizeBenchmarkData(data: BenchmarkData) {
  if (!data?.benchmarks) return data;

  const cap = computeSpeedupCap(data.benchmarks);
  const benchmarks = data.benchmarks.map((b) => {
    const rawSpeedup = b.speedup ?? 0;
    const displaySpeedup = Math.min(rawSpeedup, cap);
    return {
      ...b,
      raw_speedup: rawSpeedup,
      speedup: displaySpeedup,
      speedup_capped: rawSpeedup > displaySpeedup,
    };
  });
  ...
  return { ...data, benchmarks, summary, speedup_cap: cap };
}
...
export default function Dashboard() {
  ...
  const exportFilteredJson = () => {
    const payload = {
      ...
      normalization: benchmarkData?.speedup_cap
        ? {
            speedup_cap: benchmarkData.speedup_cap,
            note: 'Speedups are capped for UI display; raw_speedup contains the original value.',
          }
        : undefined,
    };
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <div className="bg-white/5 rounded-lg p-4 text-left text-sm font-mono text-white/70 mb-4">
            ...
            <p className="text-accent-primary">python -m tools.dashboard.server --port 6970</p>
          </div>
          <button
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <SpeedupChart benchmarks={filteredBenchmarks} speedupCap={benchmarkData?.speedup_cap} />
              </div>
              <div>
            ...
            <BenchmarkTable
              ...
              pinnedBenchmarks={pinnedBenchmarks}
              favorites={favorites}
              speedupCap={benchmarkData?.speedup_cap}
