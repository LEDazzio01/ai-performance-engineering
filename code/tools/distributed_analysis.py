...
Usage:
    ...
    python -m tools.distributed_analysis scaling --nodes 8
    python -m tools.distributed_analysis comm-profile
    python -m tools.distributed_analysis recommend --model llama-70b --nodes 4
"""
...
def main():
    parser = argparse.ArgumentParser(description="Distributed Training Analysis")
    subparsers = parser.add_subparsers(dest='command')
    
    subparsers.add_parser('topology', help='Show cluster topology')
    
    p = subparsers.add_parser('scaling', help='Scaling efficiency')
    p.add_argument('--model-params', type=float, default=70, help='Model params in billions')
    
    p = subparsers.add_parser('recommend', help='Recommend parallelism')
    p.add_argument('--model-params', type=float, default=70, help='Model params in billions')
    p.add_argument('--nodes', type=int, help='Number of GPUs')
