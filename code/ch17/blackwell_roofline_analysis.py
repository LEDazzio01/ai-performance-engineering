...
Architecture specs:
B200:
  - Peak FP32: 225 TFLOPS
  - Peak TF32: 450 TFLOPS  
  - Peak FP16: 450 TFLOPS
  - Peak FP8:  450 TFLOPS
  - HBM3e BW:  7800 GB/s

GB10:
  - Same GPU as B200
  - Additional: NVLink-C2C 900 GB/s CPU-GPU
"""
...
class ArchitectureSpecs:
    ...
    cpu_gpu_bandwidth_gbs: Optional[float] = None  # For GB10


def get_architecture_specs() -> ArchitectureSpecs:
    """Detect architecture and return specs"""
    ...
    if props.major == 12:  # Grace-Blackwell GB10
        return ArchitectureSpecs(
            ...
            cpu_gpu_bandwidth_gbs=900.0
        )
    elif props.major == 10 and props.minor == 0:  # Blackwell B200
        return ArchitectureSpecs(
            name="Blackwell B200",
            peak_fp32_tflops=225.0,
            peak_fp16_tflops=450.0,
    ...
    elif props.major == 9:  # Hopper H100
        return ArchitectureSpecs(
            name="Hopper H100",
        ...
        return ArchitectureSpecs(
            name=f"Unknown (SM {props.major}.{props.minor})",
            peak_fp32_tflops=10.0,
            peak_fp16_tflops=20.0,
            peak_fp8_tflops=0.0,
            peak_tf32_tflops=20.0,
...
class RooflineAnalyzer:
    ...
    def __init__(self):
        self.specs = get_architecture_specs()
    ...
    def analyze_kernel(
        ...
        }.get(precision, self.specs.peak_fp32_tflops)
        
        # Roofline analysis
        # Memory-bound roofline: bandwidth * arithmetic_intensity
        memory_bound_tflops = (achieved_bandwidth_gbs / 1000.0) * arithmetic_intensity
        
        # Determine bottleneck
        is_memory_bound = achieved_tflops < memory_bound_tflops * 0.8
        is_compute_bound = achieved_tflops >= memory_bound_tflops * 0.8
    ...
    def print_analysis(self, results: Dict[str, float], kernel_name: str = "Kernel"):
        ...
        print(f"  Compute:     {results['achieved_tflops']:8.2f} TFLOPS "
              f"({results['compute_utilization_pct']:5.1f}% of {results['peak_tflops']:.0f} TFLOPS peak)")
        print(f"  Bandwidth:   {results['achieved_bandwidth_gbs']:8.2f} GB/s "
              f"({results['memory_utilization_pct']:5.1f}% of {results['peak_bandwidth_gbs']:.0f} GB/s peak)")
        ...
        if results['is_memory_bound']:
            ...
            print(f"  Current AI: {results['arithmetic_intensity']:.2f} FLOPs/byte")
            print(f"  Ridge point: {results['ridge_point']:.2f} FLOPs/byte")
            print()
            print("  Recommendations:")
            print("    1. Increase arithmetic intensity (more compute per byte)")
            print("    2. Reduce memory traffic:")
            print("       - Use shared memory / cache blocking")
            print("       - Fuse operations to reduce intermediate results")
            print("       - Use vectorized loads (float4, etc.)")
            print("    3. Improve memory access patterns:")
            print("       - Ensure coalesced access")
            print("       - Avoid bank conflicts")
            print("       - Use TMA for bulk transfers (Blackwell)")
        else:
            print("  Status: ⚡ COMPUTE-BOUND")
            print(f"  Compute utilization: {results['compute_utilization_pct']:.1f}%")
            print()
            print("  Recommendations:")
            print("    1. Increase compute utilization:")
            print("       - Better occupancy (more warps)")
            print("       - Reduce warp divergence")
            print("       - Use tensor cores for matmul")
            print("    2. Consider lower precision:")
            if self.specs.peak_fp16_tflops > self.specs.peak_fp32_tflops:
                print(f"       - FP16: {self.specs.peak_fp16_tflops:.0f} TFLOPS "
                      f"({self.specs.peak_fp16_tflops / self.specs.peak_fp32_tflops:.1f}x faster)")
            if self.specs.peak_fp8_tflops > 0:
                print(f"       - FP8:  {self.specs.peak_fp8_tflops:.0f} TFLOPS "
                      f"({self.specs.peak_fp8_tflops / self.specs.peak_fp32_tflops:.1f}x faster)")
        
        print("=" * 80)
    
    def get_optimization_priority(self, results: Dict[str, float]) -> str:
        """Determine optimization priority"""
        if results['is_memory_bound']:
            if results['memory_utilization_pct'] > 70:
                return "HIGH_PRIORITY_MEMORY"
            else:
                return "MEDIUM_PRIORITY_MEMORY"
        else:
            if results['compute_utilization_pct'] < 30:
                return "HIGH_PRIORITY_COMPUTE"
            elif results['compute_utilization_pct'] < 60:
                return "MEDIUM_PRIORITY_COMPUTE"
            else:
                return "WELL_OPTIMIZED"


def benchmark_example_kernels():
    """Benchmark example kernels and analyze"""
    print("=" * 80)
    print("Blackwell Roofline Analysis - Example Kernels")
    print("=" * 80)
    print()
    ...
    print(f"HBM3e:        {analyzer.specs.memory_bandwidth_gbs} GB/s")
    print()
    ...
    for _ in range(100):
        y.copy_(x)
    torch.cuda.synchronize()
    elapsed_ms = (time.perf_counter() - start) * 10.0  # avg per iteration
    
    # Analysis
    flops = 0  # No compute, just copy
    bytes_transferred = 2 * N * 4  # Read + write
    
    # Fake some FLOPs for analysis (mul by 1.0)
    flops = N
    ...
    M, K, N_dim = 4096, 4096, 4096
    A = torch.randn(M, K, device='cuda', dtype=torch.float32)
    B = torch.randn(K, N_dim, device='cuda', dtype=torch.float32)
    C = torch.empty(M, N_dim, device='cuda', dtype=torch.float32)
    ...
    for _ in range(50):
        C = torch.matmul(A, B)
    torch.cuda.synchronize()
    elapsed_ms = (time.perf_counter() - start) * 20.0  # avg per iteration
    ...
    # Example 3: Element-wise operation (low arithmetic intensity)
    print("\n" + "=" * 80)
    ...
    # Analysis
    flops = N  # One comparison per element
    bytes_transferred = 2 * N * 4  # Read + write
