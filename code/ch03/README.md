...
python cli/aisp.py bench list-targets --chapter ch03
python cli/aisp.py bench run --targets ch03 --profile minimal
```
...
- Via `aisp bench`, pass per-target flags with `--target-extra-arg` to tune NICs and reserved CPUs: `python cli/aisp.py bench run --targets ch03:rack_prep --target-extra-arg 'ch03:rack_prep=--apply --reserve 2 --nic enp193s0f0np0'`.
